...
$\sqrt{t}$ (measured ratio $1.500$ at every even power of two, to
$t=2^{17}$): bounded population, unbounded memory, never repeating.}
\label{fig:plane}
\end{figure}
...
\section{Citation: laws that read other laws by name}
Chapters one and two vary what a law \emph{does}. The guard has gone
unquestioned throughout, and it reads only \emph{presence}: a law can see that
something stands nearby, never what it is. Real statutes cite by name---``while
section~5 is in force and section~9 has been repealed''---so let $g_k,h_k$ range
over $K\cup\{\ast\}$ as in \S2.2. One theorem dies, and it is the one that had
survived everything else.

The mechanism is a single observation. Under occupancy guards \emph{no law is
ever unconditionally active}: an occupied exception cell always blocks it. That
fact \emph{is} Gridlock. Cite a kind nothing ever enacts and the exception
clause is vacated, so the interior of a written code can act---in a solid block
of ten laws, occupancy guards leave exactly one active law and a citation guard
leaves all ten.
\begin{theorem}[Gridlock's epitaph]
Every uniform solid region is interior-frozen if and only if
$h_k\in\{\ast,k,g_k\}$ for every kind $k$. The fraction of constitutions
satisfying this is $\big((3n+1)/(n+1)^2\big)^n$: $60.49\%$ at $n=2$, $24.41\%$
at $n=3$, $7.31\%$ at $n=4$, $0.34\%$ at $n=6$.
\end{theorem}
\noindent Gridlock does not so much die as \emph{thin out}: still the rule for
small vocabularies, vanishing as the vocabulary grows. Everything else survives
verbatim---Out-Degree, Tropical Speed, Path-Sum Confinement, Anchor, Balance,
Dead Letter under OR---though not for the reason one expects: citation makes
strictly \emph{more} laws active, and the monovariant survives because it never
reads the guard at all. What does fall with Gridlock is the $\mathbb{F}_2$-linear
layer, since the step map is now a degree-3 polynomial in the kind fields rather
than a function of occupancy.

The sector is \emph{cellular-automaton complete}: all $256$ elementary CAs
simulate exactly, cell for cell, at $\le15$ kinds and window~$1$. Three
specimens. \textsc{Lacuna}: a ring written completely except for one missing
provision, where the \emph{hole} travels at exactly one cell per step,
$\Phi=\mathrm{rot}_{+1}$, while the occupancy never changes at all.
\textsc{Ledger}: a one-dimensional binary counter of card~$5$ forever, reach
$\sqrt t$, exact at $t=4^j$ through $t=65{,}536$---and the same rules with the
citations stripped are a dead letter at $t=0$. \textsc{Pascal}: Sierpinski on
$\mathbb{Z}$, $|S_t|=2^{\mathrm{popcount}(t)}$, from own-kind out-degree-one
rules---chapter one's tamest corner, opened by citation alone.

A complete weighted census of the two-kind window-one sector on $\mathbb{Z}$
($944{,}784$ constitutions, all $48$ seeds of span $\le3$, both resolutions;
$45{,}349{,}632$ certified classifications per resolution) reads: fixed
$61.5\%$, cyclic $17.6\%$, \emph{balanced} $16.0\%$, growing $3.2\%$, undecided
$1.0\%$, extinct $0.7\%$, glider $0.029\%$. One citation code in six is fixed
